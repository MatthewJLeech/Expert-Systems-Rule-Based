# %% creating true/false scenarios to see what the issue is
...
with ruleset("diagnosing_automotive"):
    ...
    @when_all((m.lights_come_on == False) & (m.engine_turns_over == False))
    def problem_is_battery_or_cables(c):
        print(c.m)
        print("The problem is battery or cables.")
    
    # problem_is_starter_motor ?
    @when_all((m.lights_come_on == True) & (m.engine_turns_over == False))
    def problem_is_spark_plugs(c):
        print(c.m)
